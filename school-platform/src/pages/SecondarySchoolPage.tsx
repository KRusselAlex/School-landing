...
import { GraduationCap, Globe, Microscope, Monitor, Calendar, Music, Languages, BookOpen } from 'lucide-react';
...
function FeatureCard({ icon, titleEn, titleFr, descriptionEn, descriptionFr, delay }: FeatureCardProps) {
  ...
  const features = [
    {
      ...
      descriptionEn: "A nurturing atmosphere that promotes academic excellence and personal growth.",
      descriptionFr: "Une atmosphère stimulante qui favorise l'excellence académique et l'épanouissement personnel."
    },
    {
      icon: <Languages className="w-14 h-14" />,
      titleEn: "German & Spanish from Form 1",
      titleFr: "Allemand et Espagnol dès la 6ᵉ",
      descriptionEn: "Students begin learning German and Spanish from their first year of secondary school.",
      descriptionFr: "Les élèves commencent à apprendre l'allemand et l'espagnol dès la première année du secondaire."
    },
    {
      icon: <Calendar className="w-14 h-14" />,
      titleEn: "Supervised Study Sessions",
      titleFr: "Sessions de Travail Supervisées",
      descriptionEn: "Every Saturday and during holidays, students benefit from guided study sessions.",
      descriptionFr: "Chaque samedi et pendant les vacances, les élèves bénéficient de sessions d'étude encadrées."
    },
    {
      icon: <Music className="w-14 h-14" />,
      titleEn: "Event Hall for Activities",
      titleFr: "Salle de Fête pour Activités",
      descriptionEn: "A dedicated space for extracurricular activities, performances, and events.",
      descriptionFr: "Un espace dédié aux activités parascolaires, spectacles et événements."
    },
    {
      icon: <Monitor className="w-14 h-14" />,
      titleEn: "Multimedia Room",
      titleFr: "Salle Multimédia",
      descriptionEn: "Modern multimedia room equipped with video projectors for interactive learning.",
      descriptionFr: "Salle multimédia moderne équipée de vidéoprojecteurs pour un apprentissage interactif."
    },
    {
      icon: <Microscope className="w-14 h-14" />,
      titleEn: "Science Laboratory",
      titleFr: "Laboratoire Scientifique",
      descriptionEn: "Well-equipped lab for hands-on experiments and practical science education.",
      descriptionFr: "Laboratoire bien équipé pour des expériences pratiques et l'enseignement scientifique."
    }
  ];
              ...
              <Card className="h-full hover:shadow-xl transition-shadow duration-300 border-l-4 border-l-secondary">
                <CardContent className="p-8">
                  ...
                  <div className="space-y-3">
                    {['6ᵉ', '5ᵉ', '4ᵉ', '3ᵉ', '2ⁿᵈᵉ', '1ʳᵉ', 'Terminale'].map((grade, index) => (
                      <motion.div
                        key={grade}
            ...
            >
              <Card className="h-full hover:shadow-xl transition-shadow duration-300 border-l-4 border-l-primary">
                <CardContent className="p-8">
                  ...
                  <div className="space-y-3">
                    {['Form 1', 'Form 2', 'Form 3', 'Form 4', 'Form 5', 'Lower Sixth', 'Upper Sixth'].map((grade, index) => (
                      <motion.div
                        key={grade}
                        initial={{ opacity: 0, x: 20 }}
                        whileInView={{ opacity: 1, x: 0 }}
        ...
        <div className="container mx-auto px-4">
          ...
          >
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end p-6 opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="text-white">
                  <h4>{language === 'en' ? 'Modern Library' : 'Bibliothèque Moderne'}</h4>
                  <p className="text-sm">{language === 'en' ? 'Extensive resources' : 'Ressources étendues'}</p>
                </div>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end p-6 opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="text-white">
                  <h4>{language === 'en' ? 'Science Laboratory' : 'Laboratoire Scientifique'}</h4>
                  <p className="text-sm">{language === 'en' ? 'Hands-on experiments' : 'Expériences pratiques'}</p>
                </div>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end p-6 opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="text-white">
                  <h4>{language === 'en' ? 'Modern Campus' : 'Campus Moderne'}</h4>
                  <p className="text-sm">{language === 'en' ? 'World-class facilities' : 'Installations de classe mondiale'}</p>
                </div>
              </div>
            </motion.div>
          </motion.div>
      ...
      >
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          <h2 className="text-4xl mb-6 text-white">
            {language === 'en' ? 'Join Our Academic Excellence' : 'Rejoignez Notre Excellence Académique'}
          </h2>
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            ...
            >
              {language === 'en' ? 'Download Brochure' : 'Télécharger la Brochure'}
            </Button>
          </div>
        </div>
      </motion.section>
